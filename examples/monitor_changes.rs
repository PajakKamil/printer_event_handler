...
async fn main() -> Result<(), PrinterError> {
    ...
    monitor.monitor_printer(&target_printer_name, 10, |current, previous| {
            ...
            if prev != current {
                ...
                if prev.printer_status_code() != current.printer_status_code() {
                    println!("   └── PrinterStatus: {:?} → {:?}", 
                        prev.printer_status_code().map(|c| format!("{} ({})", c, 
                            prev.printer_status_description().unwrap_or("Unknown"))),
                        current.printer_status_code().map(|c| format!("{} ({})", c,
                ...
                if prev.printer_state_code() != current.printer_state_code() {
                    println!("   └── PrinterState: {:?} → {:?}", 
                        prev.printer_state_code().map(|c| format!("{} ({})", c,
                            prev.printer_state_description().unwrap_or("Unknown"))),
                        current.printer_state_code().map(|c| format!("{} ({})", c,
                ...
                if prev.detected_error_state_code() != current.detected_error_state_code() {
                    println!("   └── DetectedErrorState: {:?} → {:?}", 
                        prev.detected_error_state_code().map(|c| format!("{} ({})", c,
                            prev.detected_error_state_description().unwrap_or("Unknown"))),
                        current.detected_error_state_code().map(|c| format!("{} ({})", c,
                            current.detected_error_state_description().unwrap_or("Unknown")))
                    );
                }
                
                // ExtendedPrinterStatus changes
                if prev.extended_printer_status_code() != current.extended_printer_status_code() {
                    println!("   └── ExtendedPrinterStatus: {:?} → {:?}", 
                        prev.extended_printer_status_code().map(|c| format!("{} ({})", c,
                            prev.extended_printer_status_description().unwrap_or("Unknown"))),
                        current.extended_printer_status_code().map(|c| format!("{} ({})", c,
                            current.extended_printer_status_description().unwrap_or("Unknown")))
                    );
                }
